...
**Lenny** is here, carefully dusting a collection of action figures housed in a display **cabinet** to the left of the counter.`,exits:[{dir:['west','Ascii Arena','Text Games'],id:'text-games'},{dir:['east','Pixel Paradise','Graphical Games'],id:'graphical-games'},{dir:['south','out','exit'],id:'exit'},],items:[{name:['crate','discount crate','discounted games'],desc:'Ths discount crate is empty at the moment.'},{name:['cabinet','display cabinet','action figures'],desc:`You lean over and inspect the display cabinet. There are some real gems in there.
...
}`,onSelect:()=>{const hasTalked=getCharacter('lenny').chatLog.includes('cabinet');},},...window.games.map((game)=>{const shortName=getShortName(game.title)
